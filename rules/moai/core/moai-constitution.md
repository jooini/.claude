...
- [HARD] All user-facing questions that require a structured MoAI choice SHOULD go through AskUserQuestion, except where the global AskUserQuestion serialization guard requires Markdown questions for low-risk/non-ASCII interactions
...
- First option in every AskUserQuestion MUST be the recommended choice, marked "(Recommended)" in the tool payload. Localized labels such as "(권장)" may be shown in surrounding Markdown, not in the tool payload when ASCII-only mode is required.
