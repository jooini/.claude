...
If the intent remains ambiguous after all priority checks, use AskUserQuestion to present the top 2-3 matching workflows and let the user choose. In Korean or other non-ASCII conversations, keep the AskUserQuestion payload fields (`question`, `header`, `label`, `description`) ASCII-only and put localized explanation in normal Markdown text outside the tool payload, because the global AskUserQuestion serialization guard takes precedence.
...
Before executing plan, run, sync, fix, loop, or default workflows, verify project documentation exists by checking for `.moai/project/product.md`. If product.md does NOT exist, use AskUserQuestion to ask the user. The tool payload MUST remain ASCII-only; if the user's conversation_language is non-ASCII, add the localized explanation in normal Markdown before or after the tool call.
...
- Tool payload fields MUST be ASCII-only when the global AskUserQuestion serialization guard is active; localized user-facing explanation belongs in Markdown outside the tool payload.
