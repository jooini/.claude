...
[HARD] Structured interview choices SHOULD use AskUserQuestion, but the tool payload fields (`question`, `header`, `label`, `description`) MUST remain ASCII-only when the global AskUserQuestion serialization guard is active. Put localized explanation in normal Markdown outside the tool payload.
