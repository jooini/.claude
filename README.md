...
│   ├── branch-switch-detect.sh    # Git 브랜치 전환 감지
...
│   ├── cwd-change-detect.sh       # 작업 디렉토리 변경 감지
...
│   ├── gemini-auto-scan.sh        # Gemini 자동 코드 스캔
...
| PostToolUse:Bash | tool-usage-log | 도구 사용 로깅 |
| PostToolUse:Bash | branch-switch-detect | Git 브랜치 전환 감지 |
| PostToolUse:Bash | cwd-change-detect | 작업 디렉토리 변경 감지 |
| PostToolUse:Bash | gemini-auto-scan | Gemini 자동 코드 스캔 |
...
| 모듈 | 설명 | 필요 CLI |
|------|------|----------|
| gemini | Phase 0 스캔, 자동 리뷰 | `npm i -g @google/gemini-cli` |
