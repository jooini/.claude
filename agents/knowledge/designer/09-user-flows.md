...
> 원본: https://ironact.gitbook.io/ironact-docs/CrEhRPJQJpia3xh9iqbi/knowledge-designer/user-flows
...
[○ 시작]
    │
[□ 랜딩 페이지] → [□ 회원가입 폼] → [▭ 이메일 인증 발송] → [□ 이메일 확인 안내]
                                                                    │
                                                              ◇ 이메일 인증 완료?
                                                              ├─ Yes → [□ 프로필 설정] → [○ 대시보드]
                                                              └─ No  → [□ 재발송 안내]
...
Goals, Operators, Methods, Selection rules. 각 태스크의 인지/물리적 단계를 분석하여 클릭 수, 인지 부하 측정 → 효율성 최적화에 사용.
