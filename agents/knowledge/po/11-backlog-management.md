...
> 원본: https://ironact.gitbook.io/ironact-docs/CrEhRPJQJpia3xh9iqbi/knowledge-po/backlog-management

---

## Product Backlog 관리 원칙

1. **작고 관리 가능하게 유지** — 이상적 크기: 활성 아이템 30-50개. 100개 이상이면 과감히 정리. "6개월 이상 된 아이템은 아직도 중요한가?" 정기 점검
2. **항상 우선순위가 정해져 있어야** — 상위 10개는 상세하고 바로 개발 가능(Ready), 중간은 대략적 scope, 하위는 아이디어 수준 OK
3. **Outcome과 연결** — 모든 아이템은 현재 분기 Outcome과 연결 가능해야. 연결 불가면 → Parking Lot 또는 삭제
...
DoR/DoD는 팀과 합의해야 함 — "완료"의 기준이 사람마다 다르면 혼란 발생. 정기적으로 리뷰하고 팀 성숙도에 맞게 업데이트.
