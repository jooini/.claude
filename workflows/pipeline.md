...
## Phase 0: Gemini/Antigravity 스캔 (2-pass 분석)

> 모든 파이프라인의 최초 단계. S 규모에서도 실행.
> 1M 토큰 컨텍스트를 활용해 대규모 코드베이스를 한 번에 스캔.
>
> **CLI**: `${GEMINI_CLI:-agy}` 사용 (2026-06-18부터 무료/Pro/Ultra는 `gemini` deprecated → `agy` 기본).
> 모든 예시의 `gemini` 명령은 `agy`로 치환 가능 (인터페이스 동일, `-p`/`--print` 지원).

### 실행 방법

```bash
# 프로젝트 디렉토리에서 실행. 대상 경로와 질문을 상황에 맞게 조정.
"${GEMINI_CLI:-agy}" -p "다음 코드베이스를 분석하고 요약해줘:
1. 디렉토리 구조와 주요 모듈
...
- 실행 방법:
  ```bash
  "${GEMINI_CLI:-agy}" -p "다음 코드 변경사항을 리뷰해줘. 큰 문제점 위주로:
  1. 로직 오류 / 엣지 케이스 누락
  2. 성능 이슈
  3. 보안 취약점
...
- 실행 방법:
  ```bash
  "${GEMINI_CLI:-agy}" -p "다음 코드 변경사항과 관련 모듈 전체를 리뷰해줘:
...
- 실행 방법:
  ```bash
  "${GEMINI_CLI:-agy}" -p "최종 검증: 다음 전체 변경사항에서 놓친 문제가 있는지 확인해줘:
  1. 파일 간 인터페이스 불일치
  2. 누락된 import/export
  3. 타입 불일치
  4. 미완성 TODO/FIXME
